...
namespace PDollarGestureRecognizer.Editor
{
	public static class GestureDrawer
	{
		...
		public static void Save()
		{
			...
			if (path.Length != 0)
			{
				var gesture = Gesture.Build(TextUtil.GetFilename(path), SampleSize, PointBuffer.ToArray());
		...
		public static void Load()
		{
			...
			DumpGesturePointsToBuffer(gesture);
		...
		{
			var current = Gesture.Build("", SampleSize, PointBuffer.ToArray());
			
			var gestures = Resources.LoadAll<Gesture>("");
			var winner = PointCloudRecognizer.Classify(current, gestures);
		...
		{
			var gesture = Gesture.Build("", SampleSize, PointBuffer.ToArray());
			DumpGesturePointsToBuffer(gesture);
		}
		...
		private static void DumpGesturePointsToBuffer(Gesture gesture)
		{
			PointBuffer.Clear();
			_strokeId = gesture.Points[gesture.Points.Length - 1].Id + 1;
			...
			var transform = Matrix4x4.Translate(PDollarEditorWindow.Origin) * Matrix4x4.Scale(Vector3.one * scale);
			foreach (var point in gesture.Points)
			{
				PointBuffer.AddLast(new Point()
				{
					Id = point.Id,
